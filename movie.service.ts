...
export class MovieService {
  ...
  getMovies(): Movie[] {
    console.log(this.movies);  // Check if this is logging the movies correctly
    return this.movies;
  }
  ...
  {
   newMovie.imdbID = this.movies.length + 1;
   this.movies.push(newMovie);
  }

  updateMovie(updatedMovie:any)
  {
    const index = this.movies.findIndex(movie => movie.imdbID === updatedMovie.imdbID)
    if(index !== -1)
    {
